...
let modalSendEmailBtn = document.querySelector("#modal-email-submit-btn")
modalSendEmailBtn.onclick = () => {
    let modalEmailInp = document.querySelector("#modal-email")
    if (modalEmailInp.value === "") alert("Blank input email")
    else {
        let pattern = /^[^ ]+@[^ ]+\.[a-z]{3}$/
        if (modalEmailInp.value.match(pattern)) {
            alert(modalEmailInp.value)
            modalDialog.style.display = "none"
        } else {
            alert("Invalid modal email")
        }
    }
}
